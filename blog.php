...
// An archive with no article written in this language is an empty list. It
// is no longer offered in the sitemap (includes/seo.php), and this keeps a
// crawler that reaches it by following the language switcher from indexing
// the empty version.
$seoNoindex = apex_seo_noindex($seoPage) || !apex_blog_any_in_language($currentLang);
...
<script type="application/ld+json" nonce="<?= htmlspecialchars(apex_csp_nonce(), ENT_QUOTES) ?>"><?= json_encode($blogSchema, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?></script>
